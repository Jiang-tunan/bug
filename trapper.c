...
/*********************************************************************************************
 * Function_Name : procss_trap
 * Decscription : trapper 接收协议的切入点
 * Parameter :
 * 	@sock					指向 zbx_socket_t 类型的指针 与外部通信的套接字 可能用于接收trap消息或与发送trap消息相关的通信
 * 	@s						接收到的 trap 消息 内容的字符指针
 * 	@bytes_received			接收到的 trap 消息 的字节数
 * 	@ts						指向 zbx_timespec_t 类型的指针，接收到 trap 消息 的时间戳信息
 * 	@config_comms			指向 zbx_config_comms_args_t 类型的指针 接收 trap 消息的相关通信配置参数
 * 	@config_vault			指向 zbx_config_vault_t 类型的指针，表示存储与接收到的 trap 消息相关的 vault 配置信息
 * 	@config_startup_time	表示配置的启动时间
 * Return :
 * 	@ret : SUCCEED <0 成功>		FAIL <-1 失败>
 * 	@FAIL <-1 失败>
***********************************************************************************************/
		...
		int config_startup_time)
{
	...
	zbx_rtrim(s, " \r\n");	// 删除末尾字符集合 空格 回车 换行
	// zabbix_log(LOG_LEVEL_DEBUG, "trapper got '%s'\r\n", s);
	zabbix_log(LOG_LEVEL_INFORMATION, "==trapper got '%s'\r\n", s);
	if ('{' == *s)	/* JSON protocol */
	{
		...
		if (SUCCEED != zbx_json_open(s, &jp))
		{
			...
			zabbix_log(LOG_LEVEL_INFORMATION, "received invalid JSON object from %s: %s",
					sock->peer, zbx_json_strerror());
			return FAIL;
		}
		// 获取 json 对象中 “request” 字段
		if (SUCCEED != zbx_json_value_by_name(&jp, ZBX_PROTO_TAG_REQUEST, value, sizeof(value), NULL))
			return FAIL;
		zabbix_log(LOG_LEVEL_INFORMATION, "11111111111 value is  '%s'\r\n", value);
		// 获取字节量判断 
		...
		{
			 ret = scan_progress_query(sock, s, ts); 
		}
...
#define FALSE     0
#define TRUE   	  1
#define TPR_MSG_TYPE		99		// trapper 发送消息类型
#define DCR_MSG_TYPE		100		// discover 发送消息类型
int CreateQueunFlag = FALSE;
int msgid = -1;
...
// {
// 	long int mtype;                    // 消息类型 
// 	char json_buf[MAX_STRING_LEN];		// 消息存储
// };
struct msg_st
{
    long int msg_type;
...
struct msg_queue
{  
	int id;								// 所属消息队列标识	#需更改为全局变量;
	zbx_socket_t *sock; 
	zbx_timespec_t *ts;
	struct msg_st  msg_buf;
 ...
 * 
*/
// char* modify_json_key_value(const char *original_json, const char *key, 
// 								const char *new_value)
// {
//     struct zbx_json_parse jp; // 定义JSON解析结构体
//     static char modified_json[MAX_STRING_LEN]; // 定义修改后的JSON字符串
//     char *current_value; // 当前键的值

//     // 尝试解析原始JSON
//     if (SUCCEED != zbx_json_open(original_json, &jp))
//     {
//         return NULL; // 如果解析失败，返回NULL
//     }

//     // 尝试根据键名获取键值
//     if (SUCCEED == zbx_json_brackets_by_name(&jp, key, &jp))
//     {
//         current_value = zbx_json_decodevalue_dyn(jp.start + 1); // 解码键值
//     }
//     else
//     {
//         current_value = NULL; // 如果键不存在，设置为NULL
//     }

//     struct zbx_json json_data; // 定义JSON数据结构体
//     zbx_json_init(&json_data, ZBX_JSON_STAT_BUF_LEN); // 初始化JSON数据结构体
...
//         {
//             zbx_json_addstring(&json_data, name, new_value, ZBX_JSON_TYPE_STRING); // 使用新值替换当前值
//         }
//         else
//         {
//             zbx_json_decodevalue(p, value, sizeof(value)); // 解码当前键值
//             zbx_json_addstring(&json_data, name, value, ZBX_JSON_TYPE_STRING); // 保持原始值不变
//         }
//     }

//     // 将修改后的JSON复制到返回字符串
//     zbx_strlcpy(modified_json, json_data.buffer, json_data.buffer_size);

//     zbx_json_free(&json_data); // 释放JSON数据结构体
//     zbx_free(current_value); // 释放当前键值

//     return modified_json; // 返回修改后的JSON字符串
// }

/***
 * Function_Name : trapper_send_thread_function
 * Decscription :  接收进度查询消息线程
 * Parameter :
 * 	@ arg			struct msgbuf 结构体
 * Return : 
*/
void* discovery_rules_recv_thread_function(void* arg) 
{
	int result = -1;
	zbx_socket_t		sock;
	struct msg_queue* readbuf = (struct msg_queue*)arg;

	memcpy(&sock, readbuf->sock, sizeof(zbx_socket_t));
	readbuf->sock->socket = -1;
	zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::discovery_rules_recv_thread_function#, socket=%d\n",sock.socket);
	// while ( ZBX_IS_RUNNING() )
	//{      
		zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::discovery_rules_recv_thread_function#\twaiting to receive data!!!");
		// 接收消息队列中的消息
		if (-1 == msgrcv(readbuf->id, &readbuf->msg_buf, BUFSIZ, readbuf->msg_buf.msg_type, 0)) 
		{
			zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::discovery_rules_recv_thread_function#\tFailed to receive queue message");	//接收消息失败 
		}
		else
		{
			result = zbx_tcp_send_to(&sock, readbuf->msg_buf.text, readbuf->ts); // 接收消息成功 发送 json 数据
			zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::discovery_rules_recv_thread_function#, socket=%d, result=%d,msgrcv ok with:\n%s\n",readbuf->sock->socket,result, readbuf->msg_buf.text);
			
		}
		zbx_tcp_unaccept(&sock);
		// usleep(300);
	// }
	zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::discovery_rules_recv_thread_function# exit thread, socket=%d\n",readbuf->sock->socket);
    pthread_exit(NULL);
	
}

/***
 * Function_Name : scan_progress_query
 * Decscription : 扫描信息处理
 * Parameter :
 * 	@ 
 * 		
 * Return : 
*/
int scan_progress_query(zbx_socket_t *sock, char *s, zbx_timespec_t *ts)
{
	
	struct msg_st sendbuf;
	struct msg_queue msg_queue;
	pthread_t discovery_rules_recv_thread; // 接收线程
	...
	if(!CreateQueunFlag)
	{
		...
		if(msgid == -1)
		{ 
			// zbx_tcp_send_to(sock, s,tz);
			zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::scan_progress_query#\tMessage queue creation failed msgid = [%d]!",msgid);
			return FAIL;	// 消息队列创建失败
		}
		zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::scan_progress_query#\tThe message queue is created successfully! msgid = [%d], socket=%d\n", msgid,sock->socket);	// 消息队列创建成功信息

		
		zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::scan_progress_query#\tdiscovery_rules_recv_thread_function has started !");

		CreateQueunFlag = TRUE;
	}

	// 创建接收扫描进度的线程，并传递 readbuf 作为参数
		msg_queue.sock = sock;
		msg_queue.id = msgid;
		msg_queue.ts = ts;
		msg_queue.msg_buf.msg_type = TPR_MSG_TYPE;
		if (0 != pthread_create(&discovery_rules_recv_thread, NULL, discovery_rules_recv_thread_function, (void*)&msg_queue)) 
		{
			zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::scan_progress_query#\tFailed to create read message queue thread!");
			// zbx_tcp_send_to(sock, msg_queue.msg_buf.text, ts);
			return FAIL;	// 创建接收线程失败
		}

	// // 初始化发送buf
	sendbuf.msg_type = DCR_MSG_TYPE;
	zbx_strscpy(sendbuf.text, s);
	// memcpy(sendbuf.text, s, zbx_strlen_utf8(s)); // 源 json 格式
	if (msgsnd(msgid, (void *)&sendbuf, BUFSIZ, IPC_NOWAIT) == -1)
	// if(-1 == msgsnd(sendbuf.id, &sendbuf.json_buf, MAX_STRING_LEN, IPC_NOWAIT))
	{
		// zbx_tcp_send_to(sock, sendbuf.json_buf,tz);
		zabbix_log(LOG_LEVEL_INFORMATION, "Failed to send string [msgid = %d\t size = %d\t type = %d ]\n'%s' \n '%s'",msgid, BUFSIZ, sendbuf.msg_type, sendbuf.text, strerror(errno));	
		return FAIL;		// 发送失败 返回FAIL
	}

	zabbix_log(LOG_LEVEL_INFORMATION, "#TPR::scan_progress_query#\thas been sent string \n%s\n !", sendbuf.text);
	
	// // pthread_join(discovery_rules_recv_thread, NULL);
	// zbx_strscpy(sendbuf.text,buffer);
    // // 建立消息队列
    //     sendbuf.msg_type = 1; // 注意2
    //     // strcpy(sendbuf.text, buffer);
    //     // 向队列里发送数据

    //     if (msgsnd(msgid, (void *)&sendbuf, BUFSIZ, 0) == -1)
    //     {
		...
		int config_startup_time)
{
	...
	if (FAIL == (bytes_received = zbx_tcp_recv_ext(sock, 0, ZBX_TCP_LARGE))){
		return;
	}
